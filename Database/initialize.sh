...
docker run -it --rm -v "${PWD}":/scripts -e PGPASSWORD="${w_arg}" postgres /bin/sh /scripts/run-sql.sh "${d_arg}" "${a_arg}" "${p_arg}"
docker run --rm -v ${localPath}:/liquibase/changelog -w /liquibase/changelog liquibase/liquibase liquibase --changeLogFile=dbchangelog.xml --url=jdbc:postgresql://${dbHost}:${dbPort}/${dbName} --username postgres --password ${pgPass} --log-level=info update
exit 0
